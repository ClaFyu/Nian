...
function setCookie(name,value,hours,path){
    var name = escape(name);//escape可以加密数据
    var value = escape(value);//escape可以加密数据
    var expires = new Date();//过期时间
    expires.setTime(expires.getTime() + hours*3600000);//设置过期时间
    path = path == "" ? "" : ";path=" + path;//path不为空则设path=path
    _expires = (typeof hours) == "string" ? "" : ";expires=" + expires.toUTCString();//hours参数类型字符串则为空,否则转为UTCString
